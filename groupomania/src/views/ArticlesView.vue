<template>
  ...
  <div class="gifts" v-if="existedUser().token != ''">
    ...
    <div>
      <div id="chat">
        <div
          v-for="i in existedAllChat()"
          :key="i.id"
          class="chatAll"
          :id="i.id + 'chatPosition'"
        >
          <span class="chatDataTime">
            {{ i.dateTime }} &nbsp; {{ i.hour }}:{{ i.minute }}:{{ i.second }}
          </span>
          @<span class="chatName">{{ i.first_name }}</span>
          <i
            v-if="i.id_user == i.xid_user || existedUser().role == 1"
            class="fa fa-pencil fa-fw"
            @click="UpdateOneChat(i.id, i.comment)"
            title="Update"
          ></i>
          <i
            v-if="i.id_user == i.xid_user || existedUser().role == 1"
            class="fa fa-times-circle-o fa-fw rouge"
            @click="DeleteOneChat(i.id)"
            title="Delete"
          ></i>
          <div class="chatComment">
            <div
              v-if="i.id_user == i.xid_user"
              v-html="i.comment"
              class="bg"
              :id="i.id + 'chatComment'"
            ></div>
            ...
            <div v-if="i.like == 1">
              <i
                class="fa fa-thumbs-up green"
                @click="like(i.id, i.id_user)"
                :id="i.id + 'like'"
                :title="i.id + 'like'"
              ></i>
            ...
            <div v-else-if="i.dislike == 0">
              <i
                ...
                @click="like(i.id, i.id_user, i.like)"
                :id="i.id + 'like'"
                :title="i.id + 'like'"
              ></i>
              <span :id="i.id + 'likeResult'">{{ i.likes }}</span
            ...
            <div v-else>
              ...
              >&nbsp;&nbsp;
              <i
                class="fa fa-thumbs-down rouge"
                @click="dislike(i.id, i.id_user, i.dislike)"
                :id="i.id + 'dislike'"
                :title="i.id + 'dislike'"
              ></i>
              <span :id="i.id + 'dislikeResult'">{{ i.dislikes }}</span>
      ...
      <table class="table">
        <tr>
          ...
          <td style="width: 1%" id="BtnOk">
            ...
            <i
              class="fa fa-send rougeatre"
              @click="inputChat"
              @keydown.enter="inputChat"
              title="Soumettre"
              id="inputChat"
            ></i>
            <!--<button id="inputChat" @click="inputChat">OK</button>-->
            <i
              class="fa fa-edit rougeatre"
              @click="UdateOneChatNow()"
              title="Update"
              id="updateChat"
            ></i>
          </td>
...
export default {
  ...
  methods: {
    ...
    submitFile() {
      ...
      alert(JSON.stringify(formData.file));
      const headers = { "Content-Type": "multipart/form-data" };
      axios
        .post("http://localhost:3000/api/chat/images", formData, { headers })
        .then((res) => {
          if (res.ok) {
        ...
        .catch(function (err) {
          alert("Une Erreur est survenue: Images  " + err.message);
        });
    },
    submitFile1() {
      const formData = new FormData();
      //alert(xid + " :: " + this.Images.size);
      formData.append("file", this.Images);
      //const headers = { "Content-Type": "multipart/form-data" };
      /*axios
        .post("http://localhost:3000/images", formData, { headers })*/
    ...
    existedUser() {
      ...
      if (a != "") {
        ...
        a = JSON.parse(a);
        if (a[0].token == "") this.$router.push("/login");
        return a[0];
      } else {
       ...
       body: JSON.stringify(z)*/,
          ...
          .then(function (value) {
            ...
            for (let i of value.v) {
              ...
              let idCurrentUser = JSON.parse(localStorage.getItem("oneUser"))[0]
                .id;
              var likes = 0;
              var dislikes = 0;
              if (i.userlike != null && i.userlike != "[null]") {
                for (let j of JSON.parse(i.userlike)) {
                  if (j.like == 1 && j.id_user == idCurrentUser) {
                    likes = 1;
                    break;
                  }
                  if (j.dislike == 1 && j.id_user == idCurrentUser) {
                    dislikes = 1;
                    break;
                  }
                }
    ...
    UpdateOneChat(id, comment) {
      ...
      eltComment.focus();
      eltComment.innerHTML = document.getElementById(
        id + "chatComment"
      ).innerHTML;
      ...
      if (id > 0)
        fetch(`http://localhost:3000/api/chat/${id}`, {
          ...
          .then(function (value) {
            ...
            let eltInputChat = document.getElementById("inputChat");
            let eltUpdateChat = document.getElementById("updateChat");
            eltInputChat.style.display = "block";
            eltUpdateChat.style.display = "none";

            document.getElementById(value.id_now + "chatComment").innerHTML =
              value.comment_now;
    ...
    dislike(id, id_user) {
      ...
      if (eltDislike.getAttribute("class") == "fa fa-thumbs-down") {
        ...
        //likeResult = eltResult.textContent++;
        if (eltDislikeResult.textContent == "")
          eltDislikeResult.textContent = 0;
        ...
        eltDislike.disabled = true;
        eltDislikeResult.textContent--;
        //if (eltDislikeResult.textContent == 0)
        //  eltDislikeResult.textContent = "";
        dislike = 0;
      }
      let xid = JSON.parse(localStorage.getItem("oneUser"))[0].id;
